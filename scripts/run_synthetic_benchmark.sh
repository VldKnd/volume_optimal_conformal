#!/usr/bin/env bash
#SBATCH --job-name=run_synthetic_benchmark
#SBATCH --gres=gpu:1
#SBATCH --account=eric
...
if [[ -n "${MVCP_REPOSITORY_ROOT:-}" ]]; then
    REPOSITORY_ROOT=$(cd -- "$MVCP_REPOSITORY_ROOT" && pwd -P)
...
[[ -f "${REPOSITORY_ROOT}/scripts/run_synthetic_benchmark.py" ]] || {
    printf 'error: benchmark repository not found at: %s\n' \
        "$REPOSITORY_ROOT" >&2
    exit 2
}
...
printf '[synthetic benchmark] repository root: %s\n' "$REPOSITORY_ROOT"
printf '[synthetic benchmark] configuration path: %s\n' "$CONFIGURATION_PATH"
exec "${PYTHON_COMMAND[@]}" \
    -m scripts.run_synthetic_benchmark \
    "$CONFIGURATION_PATH" \
