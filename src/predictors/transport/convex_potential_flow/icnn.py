# code is copied from https://github.com/CW-Huang/CP-Flow/blob/main/lib/icnn.py
import torch
...
import numpy as np
from predictors.transport.convex_potential_flow.flows import ActNormNoLogdet
from predictors.transport.convex_potential_flow.functional import log_sum_exp
...
class ConvICNN(torch.nn.Module):
    ...
    def __init__(self, dim=1, dimh=16, num_hidden_layers=2, num_pooling=0):
        ...
        for _ in range(num_hidden_layers - 1):
            Wzs.append(
                PosConv2d(
                    hid_channels, hid_channels, kernel_size=3, padding=1, bias=True
                )
            )
    ...
    def forward(self, x):
        ...
        return (
            nn.functional.adaptive_avg_pool2d(z + x, 1).squeeze().sum(1, keepdim=True)
        )
...
class DenseNetICNN(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            nn.Conv2d(
                dim, num_init_features, kernel_size=3, stride=1, padding=1, bias=False
            ),
...
def test_convicnn():
    ...
    print(
        np.all((((icnn(x) + icnn(y)) / 2 - icnn((x + y) / 2)) > 0).cpu().data.numpy())
    )
...
def plot_softplus():
    ...
    plt.plot(
        xx.data.numpy(), laplace_softplus(xx).data.numpy(), label="Laplace Softplus"
    )
    plt.plot(
        xx.data.numpy(), gaussian_softplus(xx).data.numpy(), label="Gaussian Softplus"
    )
